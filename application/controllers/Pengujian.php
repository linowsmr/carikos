...
class Pengujian extends CI_Controller {
	...
	function __construct()
 	{
	   	...
	   	$this->load->model('model_pengujian','',TRUE);
 	}
 	...
 	public function index()
 	{
		...
		if($this->input->get('jurusan') != ""){
			...
			redirect('pengujian/proses');
		}
		else{
			$_SESSION['jurusanDipilih'] = "";
			redirect('pengujian/hasil');
		}
 	}
	...
	public function hasil()
	{
		...
		$this->model_pengujian->hapus_nilai();
		$this->model_pengujian->hapus_nilai_cluster();
		
		$hasilPencarian = $this->model_pencarian->pencarian_semua($kota, $minHarga, $maxHarga, $tipe, $fasilitaskos, $fasilitaskamar);
		...
		foreach($cluster as $row){
			...
			$nilaiKriteriaSatu = $nilaiDestinasi;
			...
			$nilaiKriteriaDua = $nilaiDestinasi - $nilaiKriteriaSatu;
			$this->model_pengujian->nilaiSatuDua($idCluster, $nilaiKriteriaSatu, $nilaiKriteriaDua);
			$nilaiKriteriaSatu = 0;
			$nilaiKriteriaDua = 0;
			$nilaiDestinasi = 0;
		}
		...
		foreach($hasilPencarian as $row){
			...
			$this->model_pengujian->nilaiTigaEmpat($idKos, $nilaiLuasParkir, $nilaiPenjagaKos);
		...
		if($jurusanDipilih != ""){
			$data['rekomendasi'] = $this->model_pengujian->pencarian_jurusan($kota, $minHarga, $maxHarga, $tipe, $fasilitaskos, $fasilitaskamar, $jurusanDipilih);

			$data['idJurusan'] = $jurusanDipilih;
		}
		else{
			$data['rekomendasi'] = $this->model_pengujian->pencarian($kota, $minHarga, $maxHarga, $tipe, $fasilitaskos, $fasilitaskamar);
			$data['idJurusan'] = 0;
		}
		...
		if(!empty($this->session->userdata('logged_in_akun')))
        {
			...
			$this->load->view('cek_nilai_kriteria', $data);
